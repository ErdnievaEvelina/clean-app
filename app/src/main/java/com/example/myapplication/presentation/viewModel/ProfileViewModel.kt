...
data class ProfileUiState(
    ...
    val newEmail: String = "",
    val password: String = "",
    val userHouseholds: List<UserHouseholdResponseDTO> = emptyList(),
...
sealed class ProfileAction{
    ...
    data class NewEmailChanged(val value: String) : ProfileAction()
    ...
    data object ChangeEmailClicked : ProfileAction()
    data object ConfirmEmailChange : ProfileAction()
    data object CancelEmailChange : ProfileAction()
...
class ProfileViewModel(
    ...
    fun handleAction(action: ProfileAction) {
        when(action){
            ...
            ProfileAction.ConfirmEmailChange -> changeEmailAndSync()/*changeEmail()*/
    ...
    private fun changeEmailAndSync() {
        viewModelScope.launch {
            ...
            when (val changeResult = changeEmailUseCase(
                uiState.value.newEmail,
                uiState.value.password
            )) {
                is Result.Error -> _uiState.update { it.copy(isLoading = false, error = changeResult.message) }
                Result.Loading -> {}
                is Result.Success -> {
                    when (val syncResult = syncEmailUseCase()) {
                        is Result.Error -> {_uiState.update {
                            it.copy(
                                isLoading = false,
                                error = "Email изменен в Firebase, но не синхронизирован: ${syncResult.message}"
                            )
                        }}
                        Result.Loading -> {}
                        is Result.Success -> {
                            delay(3000)
                            val updatedUser = syncResult.data
                            _uiState.update {
                                it.copy(
                                    isLoading = false,
                                    user = updatedUser,           // ← Обновляем пользователя
                                    email = updatedUser.email,    // ← Обновляем email
                                    newEmail = "",                // ← Очищаем поле
                                    password = "",                // ← Очищаем поле
                                    success = true
                                )
                           }
                            loadProfile()
